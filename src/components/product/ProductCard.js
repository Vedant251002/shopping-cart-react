...
const ProductCard = ({ product, onDelete = null }) => {
    ...
    return (
      <div className="flex items-center">
        {[...Array(fullStars)].map((_, i) => (
          <svg key={`full-${i}`} className="w-4 h-4 text-accent-500" fill="currentColor" viewBox="0 0 20 20">
  ...
  return (
    <div 
      className="card group max-w-xs rounded-xl overflow-hidden shadow-md bg-white hover:shadow-xl transition-all duration-300 animate-fade-in cursor-pointer"
    ...
    >
      ...
      {/* Product details */}
      <div className="p-4">
        <h2 className="font-bold text-lg text-secondary-800 line-clamp-2 h-14 mb-1 group-hover:text-primary-600 transition-colors">
          {product.name}
        </h2>
        ...
        <p className="text-sm text-secondary-600 line-clamp-2 h-10 mb-3">
          {product.description}
        </p>
